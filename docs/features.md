...
总体结论：**协议核心、USM 安全栈、UDP/TCP/TLS 传输、v2/v3 trap/inform、基础 agent 与四个 live MIB 子树已端到端打通**。主要缺口集中在 **VACM、AgentX、DISMAN、持久化、RowStatus、通知发起方、大批 MIB 模块、语言绑定** 等"长尾"功能上。
...
| SNMPv1 **Trap-PDU** 结构 | ❌ | `PduType::TrapV1` 存在但其编解码返回 `Error::Protocol`；v1 trap-PDU 的 enterprise/generic-type/specific-type/agent-addr 字段未建模 |
...
| TLS | 🟡 | `snmpTLSTCPDomain`（RFC 6353）**仅安全通道**：rustls + ring，服务端证书认证；**无 mTLS、无 TSM** |
| DTLS over UDP | ❌ | |
| SSH | ❌ | `snmpSSHDomain` / `sshtosnmp` 网关 |
| Unix socket / IPC | ❌ | `snmpUnixDomain` |
| IPX / AAL5PVC | ❌ | 历史遗留传输 |
| Callback / STDIO / Alias | ❌ | 进程内/调试传输 |
| 共享 UDP 套接字 | ❌ | `snmpUDPsharedDomain` |
| systemd socket 激活 | ❌ | `sd-daemon.c` |
...
| **TEXTUAL-CONVENTION / 显示提示** | ❌ | |
| **范围 / SIZE 约束校验** | ❌ | |
| **DEFVAL** | ❌ | |
| **MODULE-COMPLIANCE 对象图** | ❌ | |
...
| **default_store（DS）运行时开关** | ❌ | `default_store.c` 未实现 |
| **持久化读写** | ❌ | `READ-PERSISTENT`/`SAVE-PERSISTENT` 回调未实现 |
...
| v1/v2c community 认证 | 🟡 | 仅**单一** community 精确匹配，无 rocommunity/rwcommunity ACL |
| v3/USM 权威引擎 | ✅ | 发现、用户存储、HMAC 验证、AES 解密、时间窗（150s）、`usmStats` 报告 |
| **handler chain（父子链/next handler）** | ❌ | 上游 `agent_handler.c` 的层级链，本仓库为扁平分发 |
| **SET 4 阶段（reserve1/reserve2/commit/undo）** | ❌ | `Mode::Set` 合并为单步提交，无 reservation/rollback/undo |
| **RowStatus 状态机（createAndGo/createAndWait）** | ❌ | `MapHandler::set` 对缺失行返回 `NoCreation` |
| **表格助手** | ❌ | `table_data`/`table_dataset`/`table_iterator`/`table_container`/`table_array`/`watcher`/`old_api`/`cache_handler`/`baby_steps`/`row_merge` 等均未实现 |
| **子树范围注册 / 冲突检测** | ❌ | |
| TLS/DTLS agent 绑定 | ❌ | agent 仅支持 UDP |

### 2.2 Master / Subagent / Proxy — ⛔ 未实现
...
| 功能 | 状态 | 说明 |
| --- | --- | --- |
| **VACM**（view-based access control，RFC 3415） | ⛔ | 无视图、无访问组、无 per-view ACL；仅 community/USM 身份认证 |

### 2.4 通知（Notifications）— 🟡 部分

| 功能 | 状态 | 说明 |
| --- | --- | --- |
| 客户端发送 v2c/v3 Trap + Inform | ✅ | `session` 中 `send_trap`/`send_inform` |
| Trap 接收器（`snmptrapd` 等价） | 🟡 | `trap/`：解码、认证、解密、ack inform；**无日志后端**、无 `-F` 格式、无转发、无 SQL、无 traphandle、无内嵌 Perl |
| **通知发起方（Notification Originator）** | ❌ | agent 端不主动发送 trap；无 `agent_trap.c` + target/notification MIB 表驱动 |
| **snmpTargetAddrTable / snmpTargetParamsTable** | ❌ | SNMP-TARGET-MIB |
| **snmpNotifyTable / snmpNotifyFilterProfileTable** | ❌ | SNMP-NOTIFICATION-MIB |
| **NOTIFICATION-LOG-MIB** | ❌ | |
| snmpv1 Trap-PDU 收发 | ❌ | 见 1.1 |
...
| --- | --- | --- |
| `snmp-store` 持久目录 | ❌ | 无 |
| READ-PERSISTENT / SAVE-PERSISTENT 回调 | ❌ | 可写标量（`sysContact` 等）仅存内存，重启丢失 |
| `snmpEngineBoots` 跨重启持久化 | ❌ | 仅作为配置值 |

### 2.6 live MIB 模块（`mibgroup/`）— 🟡 仅 4 个子树

| 模块 | 已实现 | 缺口 |
| --- | --- | --- |
| **SNMPv2-MIB::system**（`1.3.6.1.2.1.1`） | 🟡 sysDescr/sysObjectID/sysUpTime/sysContact/sysName/sysLocation/sysServices | **sysORTable**（`.1.9`）未实现 |
| **IF-MIB**（`1.3.6.1.2.1.2` + `.31`） | 🟡 ifNumber、ifTable 主要列、ifXTable HC 列 | ifStackTable/ifRcvAddressTable/ifTestTable；部分列；multicast/broadcast 计数器恒 0；`ifIndex` 为按名排序合成值；speed/admin/oper 硬编码 |
| **HOST-RESOURCES-MIB**（`1.3.6.1.2.1.25`） | 🟡 hrSystem/hrStorageTable/hrDeviceTable(CPU+disk)/hrProcessorTable/hrFSTable/hrSWRunTable/hrSWRunPerfTable | hrPrinterTable/hrDiskStorageTable/hrPartitionTable/hrNetworkTable/hrSWInstalledTable；hrFSLastFullBackupDate；hrSWRunStatus 写；hrFSType 恒 Other |
| **UCD-SNMP-MIB**（`1.3.6.1.4.1.2021`） | 🟡 memory(.4)/laTable(.10.1)/dskTable(.9.1)/ssCpu(.11 部分) | extTable/exec(.8)、prTable(.2)、file(.3)、version(.1)、systemStats 大部分列、dlmod、snmperrs、mrTable、logMatch、pass/pass_persist |

### 2.7 未实现的 MIB 模块（上游数百个）

下列上游 `agent/mibgroup/` 中的模块在本仓库**全部缺失**（MIB 文件已随仓库分发，但**无 agent handler 实现**）：

- **mibII 核心**：ip/at/icmp/tcp/udp（含 TCP/UDP 连接表）、ipv6、route、snmp_mib、setSerialNo
- **协议 MIB**：IP-MIB、TCP-MIB、UDP-MIB、IP-FORWARD-MIB、SCTP-MIB、EtherLike-MIB、BRIDGE-MIB、INET-ADDRESS-MIB
- **框架 MIB**：SNMP-FRAMEWORK-MIB（`snmpEngine`/`snmpEngineBoots`/`snmpEngineTime` **未作为可 walk 对象**）、SNMP-MPD-MIB、usmUserTable（可写）、usmStats（计数器仅内嵌 Report，不可 walk）
- **target / notification / notification-log**：全缺
- **TLS/DTLS/SSH/TSM TM MIB**：全缺
- **DISMAN 全套**：event/schedule/expression/ping/traceroute/nslookup（见 2.8）
- **host 扩展**：见 2.6 缺口列
- **hardware/**：cpu/fsys/memory/sensors(LM-SENSORS-MIB)
- **agent 自管理**：NET-SNMP-AGENT-MIB、NET-SNMP-EXTEND-MIB（`extend`/`pass`/`pass_persist`）、nsCache/nsDebug/nsLogging/nsModuleTable/nsTransactionTable/nsVacm
- **tunnel/**：TUNNEL-MIB
- **smux/**：SMUX + BGP/OSPF/RIP MIB
- **RMON-MIB、MTA-MIB、NETWORK-SERVICES-MIB、AGENTX-MIB**

### 2.8 DISMAN 分布式管理 MIB — ⛔ 未实现
...
| `snmptrap` | ✅ | v2c/inform；v1 trap 不支持 |
| `snmptrapd` | ✅ | 接收 + inform ack；**无 SQL/syslog/traphandle/格式化/内嵌脚本** |
| `snmptranslate` | 🟡 | **仅 `-On` 与 `-Tl`**；缺 `-Of/-Os/-OS/-Ov/-Od/-Oe/-Td/-Tp/-Ta/-Tt/-Le/-I` 等大量模式 |
| `snmpusm` | 🟡 | create/delete/activate/deactivate/changekey；缺 **list/save/cloneFrom/translate/lock** |
| `snmpvacm` | 🟡 | createview/deleteview/createsec2group/deletesec2group/createaccess/deleteaccess；**仅 client，agent 无 VACM 后端** |
| `snmpnetstat` | 🟡 | 仅 tcp/udp 连接表；缺 `-i/-r/-a/-s/-n` 接口/路由/套接字统计 |
| `snmpps` | 🟡 | 仅 hrSWRunTable 列表；缺 per-PID/`-c`/`-w` |
| `snmpd` | 🟡 | 极简 agent（mibII system + IF-MIB + HOST-RESOURCES 子集）；C 版是庞大模块化框架 |
| `snmpinform` | ✅ | 折叠为 `snmptrap --inform` |
| `encode_keychange` | ❌ | **完全缺失** |
| `snmpconf` | ❌ | **完全缺失**（交互式配置生成器） |
| `agentxtrap` | ❌ | 缺（依赖 AgentX） |
...
| 子系统 | 状态 | 说明 |
| --- | --- | --- |
| **callback 异步分发** | ❌ | `callback.c` |
| **alarm/定时事件** | ❌ | `snmp_alarm.c` |
| **mib2c 代码生成器** | ❌ | `local/mib2c` |
...
#### Task 5.1　实现 SNMPv1 Trap-PDU 收发 — **S**
...
- [ ] 单测：`pdu.rs` 对 v1 trap-PDU 做字节级往返，匹配 RFC 1157 抓包示例。
- [ ] 单测：`trap.rs` 将 6 个 generic-trap 号映射到正确的 v2 snmpTrapOID 常量（RFC 3584 表 1）。
- [ ] 集成测试：`tests/cli_notifications.rs` 新增用例，`snmptrap -v 1 ...` 发送，进程内 `TrapReceiver` 收到并断言 enterprise/generic/specific/uptime。
- [ ] 互操作：本仓库 `snmptrap -v 1` 发往**上游 net-snmp snmptrapd**，日志出现 `TRAP, SNMP v1`；反向（上游 snmptrap → 本仓库 snmptrapd）也能解析。
- [ ] `PduType::TrapV1` 在 `to_rasn` 不再返回 `Error::Protocol`。

---

#### Task 5.2　snmptranslate 扩展输出模式 — **M**
...
- [ ] 单测/集成：`tests/cli_translate.rs` 对每个新模式各一用例，断言输出子串。
- [ ] `snmptranslate -Of ifIndex`（加载 IF-MIB）→ 含 `IF-MIB::ifTable.ifEntry.ifIndex`。
- [ ] `snmptranslate -Tp -M ./mibs system` 打印 `system` 子树，含 `+-` 缩进。
- [ ] `snmptranslate -On -OS ifTable.ifEntry.ifIndex` → `.1.3.6.1.2.1.2.2.1.1`。
- [ ] `-Oe`：`snmptranslate -M ./mibs ifAdminStatus` 显示枚举。
- [ ] `-Td`（依赖 5.17）显示 DESCRIPTION。

---

#### Task 5.3　补齐 snmpusm / snmpvacm 操作 — **S**
...
- [ ] `tests/cli_mgmt.rs` 新增用例覆盖每个新子命令的参数解析与错误路径（agent 拒绝走 ExpectFail）。
- [ ] 与 5.6/5.10 联调后：`snmpusm list`（进程内 agent）返回已知用户；`snmpvacm list views` 返回已建视图。
- [ ] `mgmt.rs` 新增构造器单测（参数→VarBind 字节正确）。
- [ ] `snmpusm translate` 输出与 RFC 3414 §A.2 向量一致。

---

#### Task 5.4　实现 `encode_keychange` 工具 — **S**
...
- [ ] `tests/cli_mgmt.rs`（或新文件）：`encode_keychange -e 0x... -a SHA -E old -N new` 退出 0 且输出为 hex；长度 = `2 * (key_len)`（random+digest 各一）。
- [ ] 交叉验证：用上游 `encode_keychange` 相同输入，比对输出 random+digest 一致（random 固定为测试用常量或可 `--seed` 注入便于比对）。
- [ ] `--help` 文本列出全部选项。

---

#### Task 5.5　实现 `snmpconf` 工具 — **M**
...
- [ ] 单测：给定一组交互回答（通过 stdin 脚本），生成文件包含预期指令。
- [ ] 往返校验：生成文件被 `read_app_config` 解析，得到对应 `Directive` 集合。
- [ ] 集成：生成的 `snmpd.conf` 可被 `snmpd` 读取并应用（`tests/cli_mgmt.rs` 启动 agent 验证）。
- [ ] `--help` 与 `-i`（交互）/ `-f`（文件）模式文档。

### 优先级 P1 — Agent 框架核心能力

---

#### Task 5.6　VACM 访问控制（RFC 3415）— **XL**
...
- [ ] 单测：`vacm::is_view_accessible` 对 10 步算法的覆盖（含 mask 位匹配、excluded 视图、`exact`/`prefix` context 匹配）。
- [ ] 集成：`tests/` 新增 v2c 用户被 view 拒绝读取 `system` 的用例（返回 `noAccess` 或 varbind 为 `noSuchObject`）。
- [ ] 集成：`snmpvacm` 经 SET 建视图后，相应 OID 变可读/不可读（依赖 5.8 完成可写表后补全用例）。
- [ ] 互操作：上游 `snmpvacm` 对本仓库 agent 操作成功；上游 agent 的 `snmpd.conf` 示例（含 `com2sec`/`access`）可被本仓库 `snmpd` 加载并行为一致。
- [ ] `snmpwalk` 在无权限视图下返回空而非泄露 OID。

---

#### Task 5.7　SET 4 阶段事务（reserve1/reserve2/commit/undo）— **L**
...
- [ ] 单测：构造两个相互约束的标量（A 设为 X 时 B 必须 ≤ Y），Reserve2 检测到冲突，返回 `inconsistentValue` 且 A/B 最终值不变（Undo 生效）。
- [ ] 单测：单步默认 handler（未实现 prepare 的旧 handler）行为不变，向后兼容。
- [ ] 集成：`tests/end_to_end.rs` 多 varbind SET，其中第二个非法，断言第一个未持久化（`get` 仍为旧值）。
- [ ] `ScalarHandler::writable` 与 `MapHandler::writable` 接入 4 阶段（Reserve1 校验类型）。

---

#### Task 5.8　RowStatus 行创建语义 — **L**
...
- [ ] 单测：`createAndGo` 缺必需列 → `inconsistentName`；齐全 → 行出现且状态 `active`。
- [ ] 单测：`destroy` 已存在行 → 行消失；后续 GET 该实例 → `noSuchInstance`。
- [ ] 单测：在 `notReady` 状态 SET 非状态列 → `inconsistentValue`。
- [ ] 集成：用 5.6 VACM 表做端到端 RowStatus 行创建（`snmpvacm createview` → agent 接受）。
- [ ] 与 5.6 联调后，`snmpvacm` 全链路 create/delete 成功。
...
- [ ] 单测：`TableHandler` 对稀疏列（部分行缺某列）GETNEXT 跳到正确下一列而非 `noSuchInstance`。
- [ ] 单测：`cache_handler` 在 TTL 内不重新调用 provider，过期后刷新。
- [ ] doctest：每个助手至少一个可运行示例。
- [ ] 迁移：`mibgroup/interfaces.rs` 用 `TableHandler` 重写后行为不变（现有 `tests/end_to_end.rs` 全绿），代码量下降。
- [ ] 基准（可选）：大表（10k 行）walk 耗时较 `FnHandler` 重排版本下降。

---

#### Task 5.10　sysORTable 与框架 MIB 可 walk 对象 — **M**
...
- [ ] `snmpwalk -c public 127.0.0.1 1.3.6.1.6.3.10.2.1` 返回 4 个 snmpEngine 标量，`snmpEngineBoots` ≥ 1。
- [ ] `snmpwalk ... 1.3.6.1.6.3.15.1.1` 返回 6 个 usmStats 计数器。
- [ ] 触发一次未知用户的 v3 请求后，`snmpget ... usmStatsUnknownUserNames.0` 计数 +1。
- [ ] `snmpwalk ... system` 包含 `sysORTable` 行，列出已登记模块。
- [ ] 集成测试 `tests/end_to_end.rs` 新增用例。
...
- [ ] 集成：SET `sysContact.0 = X` → 重启 agent → GET `sysContact.0` 仍为 X。
- [ ] 集成：正常退出 3 次后 `snmpEngineBoots` = 初始值 + 3；kill -9 后不递增。
- [ ] `createUser alice ...` 写入 `snmpd.conf` 后，删除该行重启，alice 仍可用（来自持久文件）。
- [ ] 持久文件可被 `config/parse.rs` 往返解析。
- [ ] 文档：`snmpd.conf` 注明 `persistentDir` 指令支持。

---

#### Task 5.12　通知发起方 + target/notification MIB — **XL**
...
- [ ] 集成：agent 启动时配 `trap2sink 127.0.0.1 public`，进程内 `TrapReceiver` 收到 `coldStart`。
- [ ] 集成：`Agent::send_notification(customOid, [varbind])` 经 notify 表路由到多个目标（v2c + v3）。
- [ ] `snmpwalk ... snmpNotifyTable` 返回已配置目标；SET `snmpNotifyRowStatus = destroy` 后该目标不再收到 trap（依赖 5.8）。
- [ ] inform 模式：目标不在线时 agent 按 `snmpTargetAddrTimeout/RetryCount` 重试。
- [ ] 互操作：上游 `snmptrapd` 收到本仓库 agent 发的 v2c trap 与 v3 inform。

---

#### Task 5.13　snmptrapd 增强 — **M**
...
- [ ] `-F "%Y-%m-%d %H:%M:%S %N: %v"` 输出格式符合预期（单测格式化器）。
- [ ] `traphandle 1.3.6.1.6.3.1.1.5.1 /path/script`：收到 linkDown 时脚本被调用，stdin 收到 varbind 文本（集成测试用临时脚本）。
- [ ] sqlite 后端：表 `snmptrap` 有行（feature 开启时）。
- [ ] `snmpwalk` 到 snmptrapd 的 `nlmLogTable` 返回最近通知（依赖 5.12）。
- [ ] `-Os` syslog 输出（CI 中跳过 syslog 实际写入，验证构造的日志行）。

### 优先级 P2 — 传输安全与传输域扩展

---

#### Task 5.14　RFC 6353 Transport Security Model（TSM）— **L**
...
- [ ] 集成：客户端带证书连 mTLS agent，`securityName` 来自 CN；无证书被拒（`tlsClientCertificateUnknown`）。
- [ ] 集成：`snmpget -L tls ...`（带 `--cert`/`--privkey`）成功，agent 日志显示 mapped securityName。
- [ ] `snmpwalk ... tlstmCertToTSNTable` 返回映射规则（依赖可写表 5.8）。
- [ ] VACM（5.6）基于 TSM securityName 生效。
- [ ] 互操作：上游 `snmpd` + `snmpget -L` 与本仓库互通（证书链信任时）。

---

#### Task 5.15　DTLS over UDP 传输 — **M**
...
- [ ] 集成：client↔agent DTLS loopback，GET 成功。
- [ ] 集成：未信任证书被拒（对齐 TLS 用例）。
- [ ] mTLS over DTLS 用例。
- [ ] `snmpget udp+dtls://...` 或 `--transport dtls` 选项。

---

#### Task 5.16　Unix socket / Callback 传输 — **S**
...
- [ ] 集成：client↔agent 经 Unix socket GET 成功。
- [ ] `CallbackTransport` 单测：双向消息往返。
- [ ] `Transport` trait 不需改签名即可接入（验证抽象稳定）。
- [ ] snmpd 支持 `agentaddress unix:/var/run/snmpd.sock`。

### 优先级 P2 — SMI 解析增强

---

#### Task 5.17　SMI 语义解析增强 — **L**
...
- [ ] 单测：解析 IF-MIB，`ifAdminStatus` 的 SYNTAX 含枚举 up(1)/down(2)/testing(3)，MAX-ACCESS read-write。
- [ ] 单测：`InetAddress`（INET-ADDRESS-MIB TC）的 DISPLAY-HINT 解析正确。
- [ ] 单测：`validate_value` 对 SIZE 越界 OctetString 报错。
- [ ] `snmptranslate -Td ifIndex` 显示完整定义（联动 5.2）。
- [ ] agent 端 SET 前用 `validate_value` 校验类型/范围（联动 5.7 Reserve1）。
- [ ] 性能：`MibRegistry::load_dir("./mibs")` 含语义解析仍 <2s。

### 优先级 P3 — Master/Subagent 与 Proxy

---

#### Task 5.18　AgentX 协议（RFC 2741）— **XL**
...
- [ ] 单测：AgentX PDU 往返字节匹配 RFC 2741 示例。
- [ ] 集成：subagent 注册 `1.3.6.1.4.1.9999`，外部 `snmpget` 该子树经 master 转发到 subagent 返回值。
- [ ] 集成：subagent 断开后，其子树 GET 返回 `noSuchObject`（回收）。
- [ ] 互操作：本仓库 subagent 连上游 snmpd（master 模式），子树可被上游 `snmpget`；反向（上游 subagent 连本仓库 master）亦然。
- [ ] `agentxtrap` 经 master 发出通知到 SNMP 管理站。

---

#### Task 5.19　Proxy forwarder（RFC 3413）— **M**
...
- [ ] 集成：agent A 配 `proxy public 127.0.0.1:2161 1.3.6.1.4.1.9999`，agent B 服务该子树；`snmpget` 到 A 命中 B 的值。
- [ ] 集成：GETNEXT 经 proxy 正确翻页。
- [ ] v3 proxy：A↔B 间用 USM authPriv。
- [ ] 子树冲突检测：proxy 子树与本地注册重叠时给出告警。

---

#### Task 5.20　SMUX（RFC 1227）— **M**
...
- [ ] 单测：SMUX PDU 编解码。
- [ ] 集成：mock SMUX peer 注册子树，`snmpget` 经 agent 转发返回值（无真实路由守护时用 mock）。
- [ ] 文档：注明 SMUX 主要为历史兼容，建议新部署用 AgentX（5.18）。

### 优先级 P3 — MIB 模块长尾

---

#### Task 5.21　mibII 核心 MIB 模块 — **L**
...
- [ ] `snmpwalk ... tcp` 返回连接表；与系统 `ss -t` / `netstat` 数量一致（允许多寡差异，主键一致）。
- [ ] `snmpnetstat -p tcp`（联动 5.28）输出连接列表。
- [ ] `snmpget ... ipInReceives.0` 返回计数；重复请求值递增。
- [ ] `snmpwalk ... 1.3.6.1.2.1.11`（snmp_mib）返回 30 个标量。
- [ ] 集成测试覆盖至少 TCP/UDP 表。

---

#### Task 5.22　HOST-RESOURCES-MIB 补全 — **M**
...
- [ ] `snmpwalk ... hrFS` 中 `hrFSType` 反映真实文件系统类型（ext4/xfs/ntfs）。
- [ ] `snmpset ... hrSWRunStatus.PID i 4` 后进程退出（需权限，集成测试用自身子进程）。
- [ ] `snmpwalk ... hrDiskStorageTable` / `hrPartitionTable` 返回磁盘/分区（Linux 上）。
- [ ] 其它平台不 panic（CI 矩阵覆盖）。

---

#### Task 5.23　UCD-SNMP-MIB 补全 + extend/pass — **L**
...
- [ ] `snmpwalk ... 1.3.6.1.4.1.2021.11` 返回全部 ssCpuRaw/ssIO 列。
- [ ] `exec diskusage df -h` 后 `snmpget ... extOutput.1` 返回 df 第一行。
- [ ] `extend pingcheck /bin/ping -c1 127.0.0.1` 后 `snmpget ... nsExtendOutput1Line."pingcheck"` 返回 ping 结果。
- [ ] `pass -p 1.3.6.1.4.1.9999 /path/script` 后 walk 该子树返回脚本输出。
- [ ] pass_persist：多次 GET 复用同一子进程（脚本保持运行）。
- [ ] 集成测试覆盖 exec/extend/pass 各一。

---

#### Task 5.24　hardware/ 抽象层（cpu/fsys/memory/sensors）— **M**
...
- [ ] Linux CI：`snmpwalk ... lmTempSensorsTable` 返回至少一个温度（如无传感器则空表，不报错）。
- [ ] 重构后 `tests/end_to_end.rs` 全绿（host/ucd 输出不变）。
- [ ] trait 可被测试 mock（注入假数据）。

---

#### Task 5.25　DISMAN 分布式管理 MIB 全套 — **XL**
...
- [ ] 集成：创建 `mteTrigger` 监控某 Counter，超过阈值时 agent 发 trap（联动 5.12 接收）。
- [ ] 集成：`schedTable` 定时 Set 一个标量，按时触发。
- [ ] 集成：`pingResultsTable` 创建行后出现 ping 结果。
- [ ] DISMAN-EXPRESSION：GET `expExpression` 实例返回计算值。
- [ ] 互操作：上游配置示例可在本仓库 agent 复现。

---

#### Task 5.26　协议杂项 MIB — **M**
...
- [ ] `snmpwalk ... dot3StatsTable` 不报错（Linux 上可能仅 ifIndex 关联行）。
- [ ] 各 MIB at minimum 返回正确结构（即便值为 0/空）。
- [ ] 单测覆盖 OID 注册范围与列编号。

---

#### Task 5.27　NET-SNMP 自管理 MIB — **M**
...
- [ ] `snmpwalk ... 1.3.6.1.4.1.8072.1` 返回 nsCache/nsModule 行。
- [ ] SET `nsCacheTimeout.MODULE = 30` 后该模块缓存 TTL 改变（联动 5.9 验证）。
- [ ] `nsModuleTable` 列出所有已注册 MibHandler。
- [ ] `nsTransactionTable` 在 SET 进行时出现一行（联动 5.7，时序测试）。

### 优先级 P3 — 工具增强

---

#### Task 5.28　snmpnetstat 完整模式 — **M**
...
- [ ] `snmpnetstat -i` 输出含 lo/eth0。
- [ ] `snmpnetstat -r` 输出含默认路由。
- [ ] `snmpnetstat -s` 分 IP/TCP/UDP 段。
- [ ] `snmpnetstat -an` 全数字。
- [ ] 集成测试 `tests/cli_tables.rs` 扩展。

---

#### Task 5.29　snmpps 增强 — **S**
...
- [ ] `snmpps -c` 显示完整命令行。
- [ ] `snmpps -w` 含 CPU%/MEM%。
- [ ] `snmpps HOST 1234` 仅显示该 PID。

---

#### Task 5.30　snmpd.conf 完整指令支持 — **M**
...
- [ ] 单测：上游 `snmpd.conf.example` 全量解析不报错（未知指令跳过并 warn）。
- [ ] 集成：含 `com2sec`/`group`/`access`/`view` 的 conf 启动后 VACM 生效（联动 5.6）。
- [ ] 集成：`trapsink` 指令启动后 agent 启动发 coldStart（联动 5.12）。
- [ ] 多 `agentaddress` 同时监听 UDP + Unix。

### 优先级 P4 — 周边与生态

---

#### Task 5.31　snmpalarm / callback 异步框架 — **M**
...
- [ ] 单测：周期 alarm 按间隔触发次数正确；一次性 alarm 触发一次后移除。
- [ ] 单测：CallbackBus 多订阅者收到同一事件。
- [ ] 集成：5.25 的 schedTable 经 alarm 触发（联动验证）。

---

#### Task 5.32　mib2c 代码生成器 — **M**
...
- [ ] 对 IF-MIB `ifTable` 运行，生成的 `.rs` 可编译并注册，walk 返回结构正确的空/默认表。
- [ ] 生成代码通过 `cargo fmt` 与 `clippy -D warnings`。
- [ ] 文档：与上游 `mib2c` 用法对照表。

---

#### Task 5.33　default_store 运行时开关 — **S**
...
- [ ] 单测：set/get 往返；类别隔离。
- [ ] `defVersion` 等现有配置迁移到 DS（行为不变，回归测试）。
- [ ] `override` 指令生效。

---

#### Task 5.34　共享 UDP 套接字 + systemd socket 激活 — **S**
...
- [ ] 集成：两个 `Session` 用 shared socket 并发 GET，响应按 request-id 正确返回。
- [ ] 集成（有 systemd 环境）：`systemd-socket-activate -l 1161 ./snmpd` 继承 fd 并服务。
- [ ] 单测：request-id 路由分发正确。
...
1. **短期（P0，快速补全小缺口）**：5.1 v1 trap、5.4 encode_keychange、5.3 snmpusm/vacm 操作、5.10 框架 MIB 可 walk、5.16 Unix/Callback 传输。
2. **中期（P1，agent 生产化）**：5.6 VACM、5.7 SET 事务、5.8 RowStatus、5.9 表格助手、5.11 持久化、5.12 通知发起方。这一组解锁后续所有 MIB 模块开发。
3. **中长期（P2，广度）**：5.21 mibII 核心、5.22/5.23 host/ucd 补全、5.14/5.15 TSM/DTLS、5.17 SMI 增强、5.28/5.29 工具增强。
4. **长期（P3，长尾与生态）**：5.18 AgentX、5.19 Proxy、5.20 SMUX、5.25 DISMAN、5.26 协议杂项 MIB、5.31 alarm/callback、5.32 mib2c。

> 注：**Perl/Python 绑定、mib2c 的完整生态、snmptrapd 内嵌脚本**被 README 明确列为 out-of-scope，不在本清单内。
